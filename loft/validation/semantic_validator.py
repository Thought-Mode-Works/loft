...
class SemanticValidator:
    ...
    def validate_rule(
        ...
        if existing_rules:
            ...
            if redundancy_check["is_redundant"]:
                warnings.append(
                    f"Rule may be redundant: {redundancy_check['reason']}"
                )
    ...
    def _check_predicate_coherence(
        ...
        legal_predicates = {p for p in predicates if any(
            keyword in p for keyword in ["contract", "party", "enforce", "legal", "statute", "court"]
        )}
        tech_predicates = {p for p in predicates if any(
            keyword in p for keyword in ["server", "client", "network", "database", "api"]
        )}
        ...
        temporal_predicates = {p for p in predicates if any(
            keyword in p for keyword in ["before", "after", "during", "when", "time"]
        )}
