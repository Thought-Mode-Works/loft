...
class TestReasoningObserver:
    ...
    def test_identify_patterns_domain_specific(self, observer):
        ...
        domain_patterns = [
            p for p in patterns if p.pattern_type == PatternType.DOMAIN_SPECIFIC
        ]
...
class TestMetaReasoner:
    ...
    def test_diagnose_successful_chain(self, meta_reasoner):
        ...
        successful_chains = [
            c for c in meta_reasoner.observer.chains.values() if c.overall_success
        ]
...
class TestDiagnoseReasoningFailureFlexibility:
    ...
    def test_diagnose_with_chain_returns_same_as_string(self, meta_reasoner):
        ...
        diagnosis_from_string = meta_reasoner.diagnose_reasoning_failure(
            failed_chain.chain_id
        )
...
class TestAutoGeneratedFailurePatterns:
    """Tests for auto-generation of FAILURE patterns from error messages (issue #155)."""
    ...
    def test_single_failure_generates_error_pattern(self, observer):
        """Test that even a single failure with error message generates a FAILURE pattern."""
        steps = [
            create_step("s1", ReasoningStepType.TRANSLATION, success=True),
            create_step(
                "s2",
        ...
        # Verify the error pattern was created
        error_patterns = [
            p for p in failure_patterns if "error" in p.pattern_id or "error" in p.name
        ]
        assert len(error_patterns) >= 1

    def test_error_pattern_includes_domain(self, observer):
        """Test that error-based failure patterns include affected domains."""
        steps = [
            create_step("s1", ReasoningStepType.TRANSLATION, success=True),
            create_step(
                "s2",
        ...
        error_patterns = [
            p for p in patterns if p.pattern_type == PatternType.FAILURE and p.domains
        ]
        assert len(error_patterns) >= 1
        assert "contracts" in error_patterns[0].domains

    def test_error_pattern_includes_characteristics(self, observer):
        """Test that error patterns include error message in characteristics."""
        error_msg = "No matching rule found"
        steps = [
            create_step("s1", ReasoningStepType.TRANSLATION, success=True),
            create_step(
                "s2",
        ...
        error_patterns = [
            p
            for p in patterns
            if p.pattern_type == PatternType.FAILURE
            and p.characteristics.get("error_message")
        ]
        assert len(error_patterns) >= 1
        assert error_patterns[0].characteristics["error_message"] == error_msg

    def test_multiple_similar_errors_grouped(self, observer):
        """Test that multiple chains with same error are grouped."""
        error_msg = "Consensus timeout"

        for i in range(3):
            steps = [
                create_step(f"s{i}_1", ReasoningStepType.TRANSLATION, success=True),
                create_step(
                    f"s{i}_2",
        ...
        # Find the error pattern for consensus timeout
        consensus_error_patterns = [
            p
            for p in patterns
            if p.pattern_type == PatternType.FAILURE
            and p.characteristics.get("error_message") == error_msg
        ]
        assert len(consensus_error_patterns) == 1
        assert consensus_error_patterns[0].frequency == 3
        assert (
            len(
                consensus_error_patterns[0].characteristics.get(
                    "affected_chain_ids", []
                )
            )
            == 3
    ...
    def test_different_errors_create_separate_patterns(self, observer):
        """Test that different error messages create separate patterns."""
        errors = ["Error A", "Error B", "Error A"]
        ...
        inference_error_patterns = [
            ...
            for p in patterns
            if p.pattern_type == PatternType.FAILURE
            and ReasoningStepType.INFERENCE in p.associated_step_types
            and p.characteristics.get("error_message")
        ]

        # Should have 2 patterns (one for "Error A" with freq=2, one for "Error B" with freq=1)
        assert len(inference_error_patterns) == 2

        error_a_patterns = [
            p
            for p in inference_error_patterns
            if p.characteristics["error_message"] == "Error A"
        ]
        assert len(error_a_patterns) == 1
        assert error_a_patterns[0].frequency == 2

    def test_multiple_domains_in_pattern(self, observer):
        """Test that patterns span multiple domains."""
        error_msg = "Translation parse error"

        domains = ["contracts", "torts", "contracts"]
        ...
        assert len(error_patterns) == 1
        assert set(error_patterns[0].domains) == {"contracts", "torts"}

    def test_no_pattern_for_successful_chains(self, observer):
        """Test that successful chains don't generate error patterns."""
        steps = [
            create_step("s1", ReasoningStepType.TRANSLATION, success=True),
            create_step("s2", ReasoningStepType.INFERENCE, success=True),
        ]
        ...
        # Should have no failure patterns for error messages
        ...
        assert len(error_based_patterns) == 0

    def test_no_pattern_for_failure_without_error_message(self, observer):
        """Test that failures without error messages don't create error-based patterns."""
        steps = [
            create_step("s1", ReasoningStepType.TRANSLATION, success=True),
            create_step(
                "s2",
        ...
        # Should have no error-message-based patterns
        error_based_patterns = [
            p
            for p in patterns
            if p.pattern_type == PatternType.FAILURE
    ...
    def test_create_error_slug(self, observer):
        """Test the error slug generation."""
        # Test basic slug creation
        slug = observer._create_error_slug("Rule conflict detected")
        assert slug == "rule_conflict_detected"

        # Test long message truncation
        long_msg = "This is a very long error message that should be truncated"
        slug = observer._create_error_slug(long_msg)
        assert len(slug) <= 30

        # Test special character removal
        slug = observer._create_error_slug("Error: can't parse [value]")
        assert "[" not in slug
        assert "]" not in slug
        assert ":" not in slug

    def test_group_failures_by_error(self, observer):
        """Test the _group_failures_by_error method."""
        # Add chains with different errors
        for i, error_msg in enumerate(["Error A", "Error A", "Error B"]):
            steps = [
                create_step(
                    f"s{i}",
                    ReasoningStepType.INFERENCE,
        ...
        groups = observer._group_failures_by_error()

        assert len(groups) == 2
        key_a = (ReasoningStepType.INFERENCE, "Error A")
        key_b = (ReasoningStepType.INFERENCE, "Error B")

        assert key_a in groups
        assert key_b in groups
        assert groups[key_a]["count"] == 2
        assert groups[key_b]["count"] == 1
