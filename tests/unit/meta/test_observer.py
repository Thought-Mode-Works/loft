...
class TestAutoGeneratedFailurePatterns:
    ...
    def test_error_pattern_includes_characteristics(self, observer):
        ...
        error_patterns = [
            p
            for p in patterns
            if p.pattern_type == PatternType.FAILURE and p.characteristics.get("error_message")
        ]
        assert len(error_patterns) >= 1
    ...
    def test_multiple_similar_errors_grouped(self, observer):
        ...
        assert len(consensus_error_patterns[0].characteristics.get("affected_chain_ids", [])) == 3
    ...
    def test_different_errors_create_separate_patterns(self, observer):
        ...
        error_a_patterns = [
            p for p in inference_error_patterns if p.characteristics["error_message"] == "Error A"
        ]
